...
Одновременное отображение N счётчиков на одном полноэкранном экране. Заменяет устаревший dual-режим. На каждой карточке сразу показываются текущая партия и общий итог (из памяти работающего счётчика или последней записи в БД), далее значения обновляются через Socket.IO.
