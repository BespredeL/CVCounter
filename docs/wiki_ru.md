...
Одновременное отображение N счётчиков на одном полноэкранном экране. На каждой карточке — текущая партия, общий итог и разворачиваемый список по классам; значения обновляются через Socket.IO.
